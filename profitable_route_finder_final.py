...
class EfficientRouteFinder:
    ...
    # Items to analyze (popular trading items)
    ITEMS = {
        # Basic minerals (high volume)
        34: "Tritanium", 35: "Pyerite", 36: "Mexallon", 37: "Isogen", 
        38: "Nocxium", 39: "Zydrine", 40: "Megacyte",
        
        # Ships (commonly traded)
        587: "Rifter", 588: "Cruiser", 589: "Battleship", 590: "Industrial",
        
        # Popular modules/items
        1405: "Strip Miner", 3663: "Large Shield Extender II", 
        2048: "Damage Control II", 1952: "Energized Adaptive Nano Membrane II",
        
        # Ammo (high turnover)
        90: "Fusion S", 92: "Nuclear S", 182: "Antimatter Charge S",
        
        # Blueprint copies (often profitable)
        11134: "Venture Blueprint", 11135: "Retriever Blueprint"
    }
        ...
        for type_id, item_name in self.ITEMS.items():
            for buy_hub in self.HUBS.keys():
                for sell_hub in self.HUBS.keys():
                    ...
                    # Debug output for potential routes
                    gross_profit_temp = (sell_price - buy_price) * quantity
                    if gross_profit_temp > 1000:  # Only log potentially profitable routes
                        logger.debug(f"  Potential: {item_name} {buy_hub}→{sell_hub}: "
                                   f"{gross_profit_temp:,.0f} ISK, qty: {quantity}, "
                                   f"buy: {buy_price:.0f}, sell: {sell_price:.0f}")
                    ...
                    # Only include AI-approved routes (relaxed criteria)
                    if verdict not in ["SKIP"]:  # Allow WEAK routes too
    ...
    def save_routes_to_mongodb(self, routes: List[EfficientRoute]):
        """Save routes to MongoDB for frontend consumption"""
        if not self.mongo_service:
            logger.warning("MongoDB service not available, skipping save")
            return
        
        try:
            # Format routes for MongoDB storage (matching frontend expectations)
            formatted_routes = []
            for route in routes:
                formatted_routes.append({
                    "item_name": route.item_name,
                    "type_id": route.type_id,
                    "buy_system": route.buy_hub,
                    "sell_system": route.sell_hub,
                    "buy_price": route.buy_price,
                    "sell_price": route.sell_price,
                    "gross_profit": route.sell_price - route.buy_price,
                    "net_profit": route.net_profit,
                    "net_profit_percent": route.profit_percent,
                    "volume_available": route.quantity,
                    "competition_level": "Low",  # Default value
                    ...
                    "category": "minerals",     # Default category
                    "transport_cost": route.buy_price * route.quantity * 0.02,
                    "profit_margin": route.profit_percent / 100,
                    "ai_verdict": route.ai_verdict,
                    "ai_reasoning": route.ai_reasoning,
                })
            ...
            analysis_data = {
                ...
                "profitable_routes": len(formatted_routes),
                "avg_profit_percent": sum(r["net_profit_percent"] for r in formatted_routes) / len(formatted_routes) if formatted_routes else 0,
                "top_routes": formatted_routes[:20],  # Store top 20 routes
                "routes": formatted_routes,  # Store all routes
                "system_rankings": {
                    "Jita": "A+",
                    "Amarr": "A",
                    "Dodixie": "B+",
                ...
                "recommendations": [
                    "Focus on mineral trading for consistent profits",
                    "Monitor market data freshness (< 12 hours)",
                    "Consider volume vs margin trade-offs"
                ],
                "categories_analyzed": ["minerals", "ships"],
                "api_calls_made": self.api_calls,
                "efficiency_improvement": f"{320//max(self.api_calls,1)}x fewer API calls"
            }
            
            # Save to route_analysis_v2 collection (matches frontend expectations)
            collection = self.mongo_service.db.route_analysis_v2
            result = collection.insert_one(analysis_data)
            
            logger.info(f"✅ Saved {len(formatted_routes)} routes to MongoDB (ID: {result.inserted_id})")
            
        except Exception as e:
            logger.error(f"Error saving routes to MongoDB: {e}")
    ...
    async def find_efficient_routes(self) -> List[EfficientRoute]:
        ...
        # Step 3: Save to MongoDB for frontend
        self.save_routes_to_mongodb(routes)
        ...
        logger.info(f"  • Data saved to MongoDB for frontend")
